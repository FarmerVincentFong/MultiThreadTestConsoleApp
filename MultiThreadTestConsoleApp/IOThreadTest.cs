...
namespace MultiThreadTestConsoleApp
{
    ...
    public class IOThreadTest
    {
        ...
        private static void TestChild1()
        {
            ...
            //创建FileStream
            FileStream fs = new FileStream("testIO.txt", FileMode.OpenOrCreate, FileAccess.ReadWrite, FileShare.ReadWrite, 1024, false);
            //打印文件流打开的方式
            Console.WriteLine("filestream is {0} opened Asynchronously", fs.IsAsync ? "" : "not");
            byte[] writeBytes;
            #region 写入的信息
            string writeMsg = @"　　《道德经》全文
...
不积，既以为人，己愈有；既以与人，己愈多。天之道，利而不害。圣人之道，为而不争。〖译文〗";
            #endregion
            writeBytes = Encoding.UTF8.GetBytes(writeMsg);
        ...
        private static void EndWriteCallback(IAsyncResult res)
        {
            Thread.Sleep(500);
        ...
        private static void TestChild2()
        {
            ...
            Console.WriteLine("filestream is {0} opened Asynchronously", fs.IsAsync ? "" : "not");
            // 调用异步读取方法比信息写入到文件中
            fs.BeginRead(readBytes, 0, readBytes.Length, new AsyncCallback(EndReadCallback), fs);
            Console.ReadKey();
        }
        //结束读取的回调方法
        private static void EndReadCallback(IAsyncResult asyncResult)
        {
            ...
            string readmessage = Encoding.UTF8.GetString(readBytes, 0, readlength);
